/*
	...
	이 경우에 각 사람이 돈을 인출하는데 
	필요한 시간의 합은 3+4+8+11+13 = 39분이 된다.
	...
	줄을 서 있는 사람의 수 N과 각 사람이 돈을 인출하는데 
	걸리는 시간 Pi가 주어졌을 때, 
	각 사람이 돈을 인출하는데 필요한 시간의 합의 최솟값을 
	구하는 프로그램을 작성하시오.
	...
	모든 사람이 돈을 인출할 때 필요한 시간의 합이 가장 작은 경우를 구하는 문제입니다. 
	인출하는 데 걸리는 시간이 적은 순서대로 정렬한 뒤에 시간의 합을 구했습니다. 
*/
...
int main(void) {
    ...
    sort(a, a + n); // 필요한 시간이 낮은 순서대로 오름차순 정렬 
    for(int i = 0; i < n; i++) {
    	sum += a[i] * (n - i);
//		if(i == 0){
//			sum += a[i];
